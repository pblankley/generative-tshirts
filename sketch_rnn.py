...
class EncoderRNN(nn.Module):
    ...
    def forward(self, inputs, batch_size, hidden_cell=None):
        ...
        # hidden is (2, batch_size, hidden_size), we want (batch_size,
        # 2*hidden_size):
    ...
    def forward(self, inputs, z, hidden_cell=None):
        if hidden_cell is None:
            ...
            hidden, cell = torch.split(
                F.tanh(self.fc_hc(z)), hp.dec_hidden_size, 1)
            hidden_cell = (
...
class Model():
    ...
    def bivariate_normal_pdf(self, dx, dy):
        ...
        z_xy = (dx - self.mu_x) * (dy - self.mu_y) / \
            (self.sigma_x * self.sigma_y)
        z = z_x + z_y - 2 * self.rho_xy * z_xy
        exp = torch.exp(-z / (2 * (1 - self.rho_xy**2)))
        norm = 2 * np.pi * self.sigma_x * self.sigma_y * \
            torch.sqrt(1 - self.rho_xy**2)
        return exp / norm
    ...
    def kullback_leibler_loss(self):
        LKL = -0.5 * torch.sum(1 + self.sigma - self.mu**2 -
                               torch.exp(self.sigma)) / float(hp.Nz * hp.batch_size)
        if use_cuda:
    ...
    def conditional_generation(self, epoch):
        ...
        for i in range(Nmax):
            ...
            # decode:
            self.pi, self.mu_x, self.mu_y, self.sigma_x, self.sigma_y, \
                self.rho_xy, self.q, hidden, cell = self.decoder(
                    input, z, hidden_cell)
            hidden_cell = (hidden, cell)
    ...
    def sample_next_state(self):
        ...
        if use_cuda:
            return Variable(
                next_state.cuda()).view(
                1, 1, -1), x, y, q_idx == 1, q_idx == 2
        else:
            return Variable(next_state).view(
                1, 1, -1), x, y, q_idx == 1, q_idx == 2
